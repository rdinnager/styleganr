...
STYLEGANR_API void * c_styleganr_bias_act_autograd (void* x, void* b, void* xref, void* yref, void* dy, int grad, int dim, int act, float alpha, float gain, float clamp)
{
  //LANTERN_FUNCTION_START
  throw std::runtime_error("C++ `bias_act_autograd` is only supported on CUDA runtimes.");
  //LANTERN_FUNCTION_END
}