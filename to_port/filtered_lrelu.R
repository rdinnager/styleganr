...
filtered_lrelu <- function(x, fu = NULL, fd = NULL, b = NULL, up = 1, down = 1, padding = 0, 
    ...
    if(impl == 'cuda' & x$device$type == 'cuda') {
        return(.filtered_lrelu_cuda(up = up, down = down, padding = padding, gain = gain, slope = slope, clamp = clamp, flip_filter = flip_filter)(x, fu, fd, b, NULL, 0, 0))
    }
...
.filtered_lrelu_ref <- function(x, fu = NULL, fd = NULL, b = NULL, up = 1, down = 1, padding = 0, gain = sqrt(2), slope = 0.2, clamp = NULL, flip_filter = FALSE) {
    ...
    c(fu_w, fu_h) %<-% .get_filter_size(fu)
    c(fd_w, fd_h) %<-% .get_filter_size(fd)
    if(!is.null(b)) {
...
.filtered_lrelu_cuda <- function(up = 1, down = 1, padding = 0, gain = sqrt(2), slope = 0.2, 
    ...
    FilteredLReluCuda <- autograd_function(
        ...
        forward = function(ctx, x, fu, fd, b, si, sx, sy) {
            ...
            if(x$dtype %in% c(torch_float16(), torch_float32())) {
            ...
            if(return_code < 0) {
                ...
                so <- filtered_lrelu_act(y, si, sx, sy, gain, slope, clamp, write_signs) # Activation function and sign handling. Modifies y in-place.
